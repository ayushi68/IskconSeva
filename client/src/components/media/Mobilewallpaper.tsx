...
export default function MobileWallpaper() {
  const [activeFilter, setActiveFilter] = useState<"all" | "portrait" | "calendar" | "krishna">("all");

  const handleFilterClick = (filter: "all" | "portrait" | "calendar" | "krishna") => {
    setActiveFilter(filter);
  };

  // Filter wallpapers based on activeFilter
  const filteredWallpapersLatest = activeFilter === "all" || activeFilter === wallpapersLatest.find(w => w.tag.toLowerCase() === activeFilter)?.tag.toLowerCase()
    ? wallpapersLatest.filter(w => activeFilter === "all" || w.tag.toLowerCase() === activeFilter)
    : [];
  const filteredWallpapersArchive = activeFilter === "all" || activeFilter === wallpapersArchive.find(w => w.tag.toLowerCase() === activeFilter)?.tag.toLowerCase()
    ? wallpapersArchive.filter(w => activeFilter === "all" || w.tag.toLowerCase() === activeFilter)
    : [];

  return (
    <div className="bg-gradient-to-b from-amber-50 via-amber-100 to-white min-h-screen pb-20 text-center">
      {/* Header Section */}
      <header className="bg-[url('/mobile/header-bg.jpg')] bg-cover bg-center text-white py-16 px-4 relative">
        <div className="absolute inset-0 bg-gradient-to-b from-black/50 to-transparent"></div>
        <div className="relative">
          <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight">
            Devotional <span className="text-yellow-400">Mobile</span> Wallpapers
          </h1>
          <p className="mt-3 text-lg md:text-xl max-w-2xl mx-auto leading-relaxed">
            Carry Krishna’s blessings with you everywhere through beautiful mobile wallpapers and spiritual calendar backgrounds
          </p>
      ...
      {filteredWallpapersLatest.length > 0 && (
        <section className="mt-12 px-4">
          <h2 className="text-2xl font-semibold text-red-900 tracking-tight">May 2025 - Latest Collection</h2>
          <div className="mt-6 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-6 justify-items-center">
            {filteredWallpapersLatest.map((item, idx) => (
              ...
              >
                ...
                />
                <div className="p-4">
                  <p className="text-xs font-medium text-orange-600">{item.date}</p>
                  <p className="text-xs text-gray-500 mb-2">{item.tag}</p>
                  <p className="text-sm font-semibold text-gray-800 mb-3">{item.title}</p>
                  <Button
                    size="sm"
      ...
      {filteredWallpapersArchive.length > 0 && (
        <section className="mt-16 px-4">
          <h2 className="text-2xl font-semibold text-red-900 tracking-tight">Archive Collection</h2>
